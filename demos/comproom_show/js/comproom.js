...
function init() {
    ...
    comproom.renderer.setClearAlpha(0);

    // 摄影机飞行
    comproom.camera.flyTo({
        time: 1000,
 ...
 */
    var openingShow = {
        ...
        init: function () {
            ...
            // 注册取消事件
            comproom.on('keydown', function (event) {
                if (event.key == "Escape") {
                    that.stop();
                }
            })
        ...
        start: function () {
            ...
            comproom.query("[Group=Inside*]").forEach(function (obj) {
                obj.visible = false;
            ...
            comproom.query("[Group=Outside*]").forEach(function (obj) {
                obj.visible = false;
                obj.initPos = obj.position; // 保存一下初始位置
                obj.moveZ(10);
            });
        ...
        step: function () {
            ...
            comproom.camera.flyTo({
                ...
                complete: function () {
                    ...
                    var dealyTime = 0;
                    var objIdx = 0;
                    comproom.query(queryString).forEach(function (obj) {
                        that.objCount++;
                        obj.visible = true;
                        obj.moveTo({
        ...
        next: function () {
            ...
            if (this.curShowIdx >= this.showInfo.length) {
                ...
                comproom.camera.rotateAround({
                    position: comproom.camera.target,
                    time: 30000, // 环绕时间(毫秒)
                    angle: 360, // 旋转角度
                });
                ...
                var that = this;
                setTimeout(function () {
                    comproom.camera.stopRotateAround();
                    that.start();
                }, 30000);
                return;
            }
        ...
        stop: function () {
            comproom.query("[Group]").forEach(function (obj) {
                obj.stopMoving();
                obj.visible = true;
                obj.position = obj.initPos
            });

            comproom.camera.stopFlying();
